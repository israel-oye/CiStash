...
async function submitForm(formData) {
    try {
        ...
        if (response.status === 200) {
            const alertString = `<div class="alert alert-success mt-4 mx-5 py-2 text-center fw-bold" role="alert">
            ...
            if ("course_code" in Object.keys(field_errors)) {
                ...
                courseCodeErrorElem.textContent += courseCodeField_error
            }
            if ("course_title" in Object.keys(field_errors)) {
                const courseTitle_error = field_errors["course_title"][0]

                const courseTitleErrorElem = document.getElementById("#course-title-error")
                courseTitleErrorElem.textContent += courseTitle_error
            }
        } else {
            // status = 500
