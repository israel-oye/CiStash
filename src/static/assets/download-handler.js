 ...
 */
function construct_alert_element(bg_color, text_color, header_content, body_content) {
    const alertString = `<div class="toast ms-auto me-3 mt-3 bg-${bg_color} bg-gradient text-${text_color} fade show" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay=25000>
                                <div class="toast-header bg-${bg_color} text-${text_color}">
...
async function downloadFile(file_uuid) {
    try {
        ...
        if (response.status == 200) {
            ...
            const alertElem = construct_alert_element("success", "light", alert_header_content, alert_body_text);
...
if (downloaders.length > 0) {
    ...
    const alertElem = construct_alert_element("info", "dark", alert_header_content, alert_body_message)
        ...
        link.addEventListener("click", function(e) {
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    ...
                    const alertElem = construct_alert_element("success", "light", alert_header_content, alert_body_message);
                    add_alert(alertElem);
